...
?>
	...
	<h5 id="comments-heading"><?php comments_number(__('0 Comments'), __('1 comment'), __('% Comments'));?> on <em><?php the_title(); ?></em></h5>
...
| <a href="<?php trackback_url(true); ?>" rel="trackback"><?php _e('Trackback'); ?></a>
			<?php endif; ?>
			</p>
		<?php else: ?>
<p><a href="#respond"><?php _e('Respond'); ?></a>
			<?php if('open' == $post->ping_status): /* Trackbacks open */ ?>
				| <a href="<?php trackback_url(true); ?>" rel="trackback"><?php _e('Trackback'); ?></a>
			<?php endif; ?>
			</p>
	...
	<?php else: /* Comments closed. */ ?>
			<p class="nocomments"><?php _e('Closed'); ?></p>
	<?php endif; ?>
	...
	<?php if(have_comments()): ?>
			...
			<div id="commentsnav" class="navigation">
				<div class="prevlink"><?php previous_comments_link(__('&laquo; Older Comments')) ?></div>
				<div class="nextlink"><?php next_comments_link(__('Newer Comments &raquo;')) ?></div>
			</div>
	<?php endif; ?>
...
<div id="respond">
	...
	<h4><?php comment_form_title(__('Respond'), __('Respond to %s')); ?></h4>
	...
	<?php else : ?>
		...
		<p>
		<label><?php _e('Name:'); ?> <?php if($req) _e("<small>(required)</small>"); ?><br />
			<input name="author" id="author" value="<?php echo $comment_author; ?>" size="30" type="text" tabindex="1" />
		</label>
		</p>

		<p>
		<label><?php _e('Email:'); ?> <?php if($req) _e("<small>(required)</small>"); ?><br />
